...
class ImageClassifierHelper(
    ...
    fun classifyStaticImage(imageUri: Uri) {
        ...
        val bitmap = getBitmapImage(imageUri)

        val tensorImage = imageProcessor(bitmap)

        val results = performanceInference(tensorImage)

        notifyResults(results)
    }
    private fun getBitmapImage(imageUri: Uri): Bitmap {
        return if (checkerVersion()) {
    ...
    private fun checkerVersion(): Boolean {
    ...
    private fun performanceInference(tensorImage: TensorImage): List<Classifications>? {
